...
void use_dog_v2( std::shared_ptr<Dog> & dog){
	...
	std::cout << "shared_ptr passed by non const reference (dog name changed in function) , dog_name : " << dog->get_name() << std::endl;
    std::cout << "use count in use_dog_v2 : " << dog.use_count() << std::endl; //1 
}
...
std::shared_ptr<Dog> get_shared_ptr_v1(){
	...
	std::cout << "Internal shared , dog_name : " << dog_ptr->get_name() << std::endl;
...
int main(){
    ...
    
	{
		...
		std::shared_ptr<Dog> shared_ptr_dog_1 = std::make_shared<Dog>("Filld1");
		std::cout << "shared_ptr_dog_1 use count(before) : " << shared_ptr_dog_1.use_count() << std::endl; //1
		use_dog_v1(shared_ptr_dog_1);
		std::cout << "shared_ptr_dog_1 use count(after) : " << shared_ptr_dog_1.use_count() << std::endl; //1
	...
	*/
	...
	//		No copy is made, reference count doesn't increment in function body
	{
		...
		std::shared_ptr<Dog> shared_ptr_dog_2 = std::make_shared<Dog>("Flizy2");
	...
	/*
	Passing by non const reference :
	Constructor for dog Flizy2 called.
	shared_ptr_dog_2 use count(before) : 1
	Changing Name...
	shared_ptr passed by non const reference (dog name changed in function) , dog_name : Riol2
	Destructor for dog Riol2 called
	shared_ptr passed by non const reference (dog name changed in function) , dog_name : PuffyDefault
	use count in use_dog_v2 : 1
	shared_ptr_dog_2 use count(after) : 1
	Destructor for dog PuffyDefault called
	*/
    ...
    
	{
		...
		std::cout << "shared_ptr_dog_3 use count(before) : " << shared_ptr_dog_3.use_count() << std::endl;//1
		use_dog_v3(shared_ptr_dog_3);
		std::cout << "shared_ptr_dog_3 use count(after) : " << shared_ptr_dog_3.use_count() << std::endl; //1
	}
	/*
	Passing by const reference :
	...
	*/
	...
	Internal shared , dog_name : Internal Dog_v1
	Managed dog address(in) : 000001BC8076D6C0
	Managed dog address (out) : 000001BC8076D6C0
	...
	// Returning by value returns a new owner (or efficiently transfers ownership).
	// Returning by reference returns only an alias to the local shared_ptr, not a new owner.Once the function ends, that alias refers to an object that has already been destroyed.
